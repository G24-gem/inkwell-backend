  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE articles
       ...
       RETURNING *`,
      [title, subtitle, blocks ? JSON.stringify(blocks) : null, cover_image_url, id, authorId]
    );
    if (result.rows.length === 0) {
...
async function publishArticle(req, res) {
  try {
    ...
    const slug = article.title
      ...
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '') + '-' + Date.now();
    const wordCount = JSON.stringify(article.blocks).split(' ').length;
    ...
    const result = await pool.query(
      `UPDATE articles
       SET status = 'published', slug = $1,
         ...
         reading_time = $5, published_at = now(), updated_at = now()
